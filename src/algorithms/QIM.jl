...
inner_product(v1::AbstractMatrix{<:Number}, v2::AbstractMatrix{<:Number}) = sum(dot(v1[:, i], o * v2[:, i]) for i in 1:size(v1, 2)) / size(v1, 1)

export covmat
function covmat(ol::AbstractVector{T}, v::AbstractVecOrMat{<:Number}) where T <: Union{<:AbstractMatrix, <:Operator}
    n = length(ol)
    vs = [oi * v for oi in ol]
    am = [inner_product(v, vsi) for vsi in vs]
